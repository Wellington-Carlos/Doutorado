...
\affil[1]{\orgdiv{Department of Electrical Engineering}, \orgname{PUC-Rio}, \city{Rio de Janeiro},\country{Brazil}}
...
\affil[2]{\orgdiv{Department of Energy Resources}, \orgname{UiS}, \city{Stavanger}, \country{Norway}}
...
\affil[3]{\orgdiv{Petrobras}, \city{Rio de Janeiro}, \country{Brazil}}

%%==================================%%
%% Sample for unstructured abstract %%
%%==================================%%

\abstract{This paper presents an integrated framework for valuing oil and gas exploration and production (E\&P) projects under geological and market uncertainties using the Real Options Approach (ROA). Building on the BDH problem, the analysis introduces enhanced managerial flexibility, allowing the options to continue, buyout, or divest to be exercised at any time during the project’s lifetime. Geological uncertainty is represented through the Egg benchmark reservoir with 101 stochastic permeability realizations, while market uncertainties are modeled using the Schwartz–Smith two-factor model for oil prices and a Geometric Brownian Motion (GBM) for variable operating costs. Monte Carlo Simulation (MCS) propagates these uncertainties, and the Least Squares Monte Carlo (LSM) method determines optimal decision strategies and quantifies option values. Results indicate that expanding the exercise window for options can increase the option value by over 300\% relative to the original BDH problem. The proposed methodology provides a robust and realistic decision-support tool, enabling more informed investment choices in uncertain E\&P environment, combining advanced stochastic modeling with dynamic optimization techniques.}
...
\keywords{Real options, Geological uncertainty, Price uncertainty, BDH problem, Least Squares Monte Carlo, Uncertainty analysis.}
...
Oil and gas exploration and production (E\&P) is a multi-stage process that begins with the investigation of sedimentary basins to identify potential hydrocarbon reservoirs, progresses through the development and operation of production facilities, and concludes with site decommissioning after resource depletion. This sector is characterized by substantial capital requirements, with financial investments commonly measured in millions of US dollars and individual project budgets often exceeding one billion dollars. Despite facing a wide range of technical and economic risks, the industry offers significant opportunities for high returns on investment.

Investment opportunities characterized by substantial uncertainty are typically designated as high-risk within corporate portfolios. While elevated uncertainty is associated with greater downside potential, it also presents the possibility of achieving outcomes that exceed expectations. Although excluding high-risk projects may help avoid capital losses, such an approach can also eliminate the chance to invest in ventures with favorable expected values. Overlooking the impact of uncertainty in project evaluation can therefore result in portfolio choices that fail to maximize stakeholder value. As highlighted by \cite{ref9} and other authors, the conventional deterministic Discounted Cash Flow (DCF) approach does not adequately account for uncertainty, implicitly treating investment as a now-or-never proposition. This assumption neglects the inherent flexibility available to managers, who can adjust their decisions in response to information that emerges as the project progresses.

Value generation in investment projects arises from the interplay between uncertainty and managerial flexibility. The assessment of the value attributable to such flexibility, often described as option-like behavior, is commonly undertaken through Real Options Approach (ROA) techniques. In this study, we employ one of the most flexible and robust methodologies for option valuation: the Least Squares Monte Carlo (LSM) method. The analysis explicitly considers three sources of uncertainty: variable operating costs, oil price, and production rate. For modeling oil prices, the two-factor stochastic process introduced by \cite{ref17} was selected, as it offers an effective balance between capturing market dynamics and maintaining interpretability for decision-makers. Variable operating costs are represented using a Geometric Brownian Motion (GBM) process. The uncertainty in the production rate will be accounted for through the multiple realizations of the permeability model of the geological benchmark Egg model. Thus, this study will focus on applying the ROA to the economic evaluation of E\&P projects under geological and market uncertainties.

The remainder of the paper is organized as follows. A literature review is presented in Section~\ref{sec2}, followed by a detailed description of the problem in Section~\ref{sec23}. Section~\ref{sec4} outlines the methodology, which includes the assumption of the GBM for variable operating costs, Two-Factor model for oil prices, a geological benchmark model (Egg model), Monte Carlo Simulation (MCS) and LSM to estimate the project's value with options. The results and related discussions are presented in Section~\ref{sec5}. Finally, Section~\ref{sec6} concludes the paper with suggestions for potential future research.

\section{Literature Review}\label{sec2}

Theory of investment under uncertainty, as developed by \cite{ref1}, provides the ROA for evaluating irreversible investment decisions in uncertain environments. In the context of E\&P industry, this approach emphasizes the value of managerial flexibility in timing investments, accounting for volatility in prices, geological information, and regulatory factors. Unlike traditional DCF method, the ROA captures the strategic value of waiting and learning before committing capital.

DCF method determines the present value of each future cash flow (CF) by applying a discount rate that reflects the time value of money. Net Present Value (NPV) is then calculated as the sum of all discounted future cash flows, including both inflows and outflows. NPV method remains the primary approach for evaluating investment projects in the E\&P industry, despite well-known limitations related to market assumptions, project flexibility, and discount rate selection \citep{ref1a}. DCF method adopts a static perspective, assuming that future decisions rely solely on the information currently available, disregarding any additional insights that may emerge at the actual decision point. This assumption overlooks managerial flexibility and the incremental value generated by the capacity to adjust decisions as the project unfolds. In light of these limitations, there is an increasing demand for alternative methodologies that enable a more rigorous and accurate assessment of investment opportunities under uncertainty. Capturing this value necessitates the application ROA.
...
\cite{ref11a} examine the impact of oil price uncertainty on investment decisions in an African oil exploration and production project under a Risk Service Contract. By modeling oil prices with GBM and incorporating managerial flexibility through ROA with a binomial lattices, the analysis evaluates different decision scenarios. Results show that, when uncertainties are ignored, project development is not recommended. MCS indicate a low probability of success under price volatility. However, incorporating managerial flexibility reveals that uncertainty can add value to the project, and providing the option to invest at a larger scale maximizes the asset’s value.
...
\begin{minipage}{0.95\textwidth}
  ...
  \caption{3D Egg model: permeability distribution of the deterministic version.}
  \label{fig1}
\end{minipage}
\end{figure}
...
Egg Model, originally introduced by \cite{ref23} and further standardized by \cite{ref18}, is a widely used synthetic benchmark for reservoir simulation studies. The model comprises 25,200 gridblocks, of which 18,533 are active, forming an egg-shaped reservoir by excluding inactive gridblocks. Each gridblock is 8 by 8m with a height of 4m. The model consists of 8 injectors and 4 producers (see Figure \ref{fig3}). Its reservoir architecture features a set of meandering, high-permeability channels embedded within a low-permeability matrix, thereby representing the geological complexity of fluvial depositional environments with seven vertical layers. Due to the high degree of vertical correlation among the seven layers, the permeability fields can be cosidered almost two dimensional. The standard configuration comprises twelve wells, with eight injectors and four producers. Figure \ref{fig3} presents the absolute permeability field and the locations of all wells. This benchmark geological model is well documented in the literature. 

\begin{figure}[H]
\centering
\begin{minipage}{0.95\textwidth}
  \includegraphics[width=\textwidth]{egg_model_log_3D_well.eps}
  \caption{3D Egg model. Blue lines are an indicator of injector wells, and red ones represent the producers.}
  \label{fig3}
\end{minipage}
\end{figure}
...
In this study, it is assumed that the vertical oil production well is fully penetrating. Accordingly, since there are 3,600 gridblocks in the first layer, of which 2,491 are active cells, this assumption yields 2,491 feasible well placement scenarios, corresponding to the number of active gridblocks in the top layer of the reservoir model. Figure \ref{fig5} illustrates the spatial distribuition of permeability in the top layer for 25 realizations of the Egg model. These realizations were randomly selected from the original set of 101, which consists of one reference field and one hundred initial stochastic models. The models exhibit high-permeability channels within a low-permeability background, with each realization presenting a distinct channel pattern, showing alternative fluvial structures.
...
\begin{equation}
    \chi_{t+1} = \chi_t e^{-\kappa \Delta t} - \left(1 - e^{-\kappa \Delta t}\right) \frac{\lambda_\chi}{\kappa} + \sigma_\chi \varepsilon_\chi \sqrt{\frac{1-e^{-2\kappa\Delta t}}{2\kappa}},
...
According to \cite{ref28}, if $\varepsilon_{\xi}$ and $\varepsilon$ are two independent standard normal random variables, it is possible to introduce correlation between the price processes described in equations \eqref{eq:short_term_discrete} and \eqref{eq:long_term_discrete}. This can be achieved by defining the random variable of the second process as:
\begin{equation}
	\varepsilon_{\chi} = \varepsilon_{\xi} \rho_{\chi\xi} + \varepsilon \sqrt{1 - \rho_{\chi\xi}^{2}},
\end{equation}
where $\rho_{\chi\xi}$ denotes the correlation coefficient between the two processes.
...
\subsection{First case: BDH problem}\label{subsec51}

\cite{ref12a} introduces the BDH problem, which evaluates an oil production project whose results are then used to build the binomial decision tree approach that underpins the ROA, as described in Section \ref{sec4}. To model uncertainty, the authors use a GBM for the oil price and operating costs. After many iterations (e.g., 10,000), and using a 10\% risk-ajusted discount rate, they estimate the present value of expected cash flow at \$404 million, see Table \ref{tab1}. This result is the base value of the original BDH problem, excluding managerial flexibility or options. Figure \ref{fig13} illustrate production profile for BDH problem according to values in Tables \ref{tab1} and \ref{tab2}.
...
\end{equation}
where $c_{t,j}$ is the cash flow received at time $t$ and path $j$, $v_{t+1,j}$ is the calculated value at the next time step, and $r$ is the risk-free discount rate.

The model incorporates two sources of uncertainty: oil price and variable operating cost, both contributing to valuation risk. The present value of the project with options is calculated to be \$421~million, which was obtained using the LSM method based on simulation combined with linear regression. Figure \ref{fig15} presents the relative frequency distribution for the project's present value without the option, as well as with the options to divest, buy out, or continue, where managerial flexibility can be exercised in Year~5. Consequently, the expected value of the embedded option in the project is estimated to be \$29~million.

\begin{figure}[H]
\centering
\begin{minipage}{0.95\textwidth}
  \includegraphics[width=\textwidth]{density_NPV_1.eps}
  \caption{PV of the project (a) without options and (b) with options in Year~5.}
  \label{fig15}
\end{minipage}
\end{figure}

Value of Flexibility arises from the ability to adapt project decisions as uncertainties are progressively resolved, rather than committing to a single deterministic plan. As highlighted by \cite{ref9}, incorporating managerial flexibility, such as options to defer, expand, buyout or divest a project, enables decision-makers to both mitigate downside risks and capture upside opportunities, thereby increasing the expected value of the project. Through real-world examples, these authors demonstrate that structuring projects with flexible alternatives can add substantial value, especially in contexts where uncertainty is significant and cannot be fully eliminated through information gathering alone. Thus, evaluating and explicitly incorporating flexibility should be a fundamental part of decision analysis in uncertain environments.

In the case of the BDH problem, we consider that the options to continue, buyout, or divest can be exercised at any time from Year~1 to Year~10 of field production, rather than only in Year~5 as initially proposed by \cite{ref12a, ref12b}. The results are presented in Figure \ref{fig16}, where we clearly observe an increase in the project value to \$484~million. In other words, the option value increased from \$29~million to \$92~million, representing an approximate 317\% increase in the option value.

\begin{figure}[H]
\centering
\begin{minipage}{0.95\textwidth}
  \includegraphics[width=\textwidth]{density_NPV_2.eps}
  \caption{PV of the project (a) without options and (b) with options from Year~1 to Year~10.}
  \label{fig16}
\end{minipage}
\end{figure}

Value of Flexibility refers to the benefit of designing projects with the capability to adapt decisions as uncertainties are resolved over time. This analysis, analogous to the VoI, seeks to determine whether the expected advantage of incorporating managerial flexibility justifies its cost. Before allocating resources to acquire additional information based solely on a positive expected monetary value from a VoI analysis, it is crucial for decision-makers focused on value maximization to also evaluate the potential gains from flexible project structures \citep{ref9}.

Flexibility tends to be particularly advantageous under the following conditions \citep{ref24}:
\begin{enumerate}
    \item When reducing uncertainty through information acquisition is not feasible.
    \item When the cost of flexibility is lower than the cost of obtaining information.
    \item When it is necessary to manage residual uncertainty after information has been collected.
    \item When flexibility enables the creation of additional value.
\end{enumerate}
In general, the first three cases primarily address risk mitigation, while the last one aims to exploit upside potential associated with uncertainty.

Figure \ref{fig17} presents the results generated by the LSM algorithm using an alternative visualization. The diagram illustrates, for each year, the probability associated with each available decision—buyout, continue, or divest—conditioned on the information available at Year 0. This type of representation offers decision makers a clear understanding of the most likely actions throughout the project horizon \citep{ref27}.

As an illustrative example, Figure \ref{fig17} indicates that, in 9,930 out of 10,000 simulated paths, the optimal strategy is to buyout the partner's share right at the start of the project. From the second year onwards, the recommended decision generally shifts towards continuing the project, although the probability of this choice gradually decreases over time. Conversely, the probability of opting for divestment shows a slight upward trend as the project progresses.

\begin{figure}[H]
\centering
\begin{minipage}{0.95\textwidth}
  \includegraphics[width=\textwidth]{options_prob.eps}
  \caption{Probability of each option for the project in each year.}
  \label{fig17}
\end{minipage}
\end{figure}

Figure~\ref{fig18} presents the decision maps for four selected time periods ($t = 3, 5, 7, 9$). These maps were generated from the simulation outputs of oil price and variable operating costs, together with the optimal decisions obtained from the real options analysis. Only the alternatives \textit{continue} and \textit{divest} are represented, as the \textit{buyout} option is not available in these specific periods (see Figure \ref{fig17}). The teal regions correspond to states where the \textit{continue} alternative yields the highest project value, while the goldenrod regions indicate conditions where \textit{divest} becomes optimal.

To construct these maps, oil price (US\$/bbl) and variable operating cost (US\$/bbl) data were extracted for each selected period from the simulated paths. The optimal decision in each state was encoded numerically using mapping: \texttt{continue} = 0, \texttt{divest} = 1. A mask excluded any states corresponding to non-relevant options. The resulting state--decision pairs were used to train a \texttt{KNeighborsClassifier} ($k=1$), which predicts the optimal decision for any given combination of oil price and operating cost.

A regular grid was defined over the range of possible oil prices (10--80~US\$/bbl) and variable operating costs (3--20~US\$/bbl), using 300 subdivisions in each dimension. The trained classifier was applied to all grid points, generating a matrix of predicted decisions. This classification map was then visualized with a two-color colormap (\texttt{teal} for \textit{continue} and \texttt{goldenrod} for \textit{divest}), producing a clear separation between decision regions.

\begin{figure}[H]
\centering
\begin{minipage}{0.95\textwidth}
  \includegraphics[width=\textwidth]{decision_maps.eps}
  \caption{Decision maps for different time periods.}
  \label{fig18}
\end{minipage}
\end{figure}

The results of Figure~\ref{fig18} reveal the progressive expansion of the \textit{divest} region over time, in agreement with the probability trends shown in Figure~\ref{fig17}. At early stages ($t=3$), \textit{continue} dominates almost the entire decision space, while at later stages ($t=9$), the \textit{divest} alternative covers most scenarios, reflecting the decreasing attractiveness of project continuation under uncertainty.

Proceeding with the analysis, to highlight the value of flexibility, Figure \ref{fig19} shows the present value of the project when the options can be exercised at different periods. The darkred line represents the project's present value assuming the option can be exercised in Year~1, Year~2, and so on, up to Year~10. It can be observed that the highest values occur at the beginning and at the end of the project, at \$455 and \$454 million, respectively. The lowest project value with the option, \$421 million, is found in Year~5 (and Year~4), which corresponds precisely to the BDH Problem, as also shown in Figure \ref{fig15}.

\begin{figure}[H]
\centering
\begin{minipage}{0.75\textwidth}
  \includegraphics[width=\textwidth]{graph_options.eps}
  \caption{Present values of the project for options exercised at different times.}
  \label{fig19}
\end{minipage}
\end{figure}

Teal line in Figure \ref{fig19} illustrates the present value of the project with options as the number of years available to exercise the option increases. It starts with the project value when the option can only be exercised in Year~1, then includes Years~1 and 2, then Years~1, 2, and 3, and so on, cumulatively adding each year until reaching the case where the option can be exercised throughout the entire period, from Year~1 to Year~10. In this latter scenario, the present value of the project reaches \$484 million, which is the same value presented in Figure \ref{fig16}. This result confirms that increasing the flexibility to exercise options leads to a higher project value. Additionally, Figure~\ref{fig19} shows that, in any scenario where options are available, the present value of the project is always higher than the present value without options, which is \$392 million (dashed goldenrod line).

\subsection{Second case: 3D Egg Model}\label{subsec52}

The methodology and results developed for the BDH problem are now extended to the 3D Egg model. The main distinction lies in the treatment of production profiles: in this case, production curves are explicitly considered as an additional source of uncertainty, since multiple permeability realizations from the Egg model yield different production curves for the producer wells. In contrast, for the BDH problem, the production profile was assumed to be fixed (see Figure \ref{fig13}).

Figure \ref{fig20} shows the production rates profiles for the four producing wells (PROD1-PROD4) over a 10-year period, based on the results of 101 reservoir simulation runs for the 3D Egg model (see Figure \ref{fig3}). The thin teal curves represent the outcomes of multiple simulation realizations, capturing the variability in production behavior due to geological uncertainty. The thick darkred line denotes the mean production rate across all realizations for each well.

\begin{figure}[H]
\centering
\begin{minipage}{0.85\textwidth}
  \includegraphics[width=\textwidth]{production_all_PRODs.eps}
  \caption{Four production wells over time for each of the 101 realizations.}
  \label{fig20}
\end{minipage}
\end{figure}

All wells exhibit the typical decline pattern observed in reservoir production: high initial rates during the early years, followed by a sharp decrease, and a subsequent stabilization at low production levels toward the end of the simulated period. The spread of the individual curves around the mean highlights the variability in production performance, which is more pronounced during the initial years and tends to converge as production rates diminish. These results provide valuable insights for quantifying uncertainty in field development planning and for comparing the relative performance of each producing well under uncertain reservoir conditions.

Figure \ref{fig21}(a) shows the field-level oil production profile for the benchmark Egg model, obtained by aggregating the production from the four producing wells (PROD1–PROD4). Production rates are expressed in million barrels (MMbbl). Thin teal curves represent the results of all simulation realizations, while the thick darkred line corresponds to the mean production profile across all realizations. Similar to the individual well behavior, the aggregated field production exhibits a sharp initial decline followed by a gradual stabilization, reflecting the combined reservoir dynamics under geological uncertainty.

\begin{figure}[H]
\centering
\begin{minipage}{0.85\textwidth}
  \includegraphics[width=\textwidth]{production_field_pdf_cdf.eps}
  \caption{(a) Field production for the benchmark Egg model and (b) Probability Density Function (PDF) and Cumulative Distribution Function (CDF).}
  \label{fig21}
\end{minipage}
\end{figure}

Figure \ref{fig21}(b) presents the statistical characterization of cumulative oil production at the end of the simulation period. The histogram represents the frequency distribution of the simulated cumulative production values. Probability density function (PDF), estimated using a kernel density estimation (KDE) method, provides a smooth representation of the underlying probability distribution, allowing for a clearer visualization of the most probable production outcomes. Cumulative distribution function (CDF), plotted in darkred, indicates the probability that the cumulative oil production will be less than or equal to a given value, thus enabling the direct estimation of percentile-based production metrics (e.g., P10, P50, P90). Together, these representations quantify the uncertainty in total field production and provide probabilistic insights for decision-making.

Table \ref{tab5} presents the base case expected cash flows for the 3D Egg model. The parameters Production Level, Variable Operating Cost Rate, and Oil Price are considered the main sources of uncertainty. However, the values reported in the table correspond to mean values across realizations. Under this base case, the project value, represented by the present value of expected cash flows, amounts to approximately \$134 million. This figure represents the project’s value without considering managerial flexibility or the inclusion of real options.

The values shown in the table for the Egg model are consistent with the characteristics of a small onshore oil field with relatively modest production. Moreover, the geological features of the Egg model, characterized by high-permeability channels embedded in a low-permeability background, resemble typical meandering river patterns commonly encountered in fluvial environments \citep{ref18}. These features reinforce the interpretation that the Egg model can be regarded as a representative analog for onshore field production.

\begin{table}[h] 
\centering 
\caption{Base Case Expected Cash Flows for the 3D Egg model}
\label{tab5}
\begin{tabularx}{14.5cm}{l *{12}{>{\centering\arraybackslash}X}} 
\toprule 
\textbf{Year} & \textbf{0} & \textbf{1} & \textbf{2} & \textbf{3} & \textbf{4} & \textbf{5} & \textbf{6} & \textbf{7} & \textbf{8} & \textbf{9} & \textbf{10} \\ 
\midrule 
Production Level              &  0.736   & 1.262 & 0.518 & 0.218 & 0.136 & 0.097 & 0.073 & 0.059 & 0.052 & 0.041 & 0.012 \\ 
Variable Op Cost Rate         &  5.00   & 5.10 & 5.20 & 5.311 & 5.41 & 5.52 & 5.63 & 5.74 & 5.86 & 5.98 & 6.09 \\ 
Oil Price                     & 70.62  & 68.61 & 65.89 & 63.08 & 60.25 & 57.91 & 55.27 & 52.81 & 50.56 & 48.43 & 46.33 \\ 
\addlinespace 
Revenues                      &  51.95   & 86.61 & 34.16 & 13.74 & 8.20 & 5.61 & 4.06 & 3.09 & 2.63 & 1.97 & 0.56 \\ 
Production Cost               &  (3.98)   & (6.74) & (3.00) & (1.46) & (1.04) & (0.83) & (0.71) & (0.64) & (0.61) & (0.54) & (0.37) \\ 
\cmidrule{2-12} 
Cash Flow                     &  47.97   & 79.88 & 31.16 & 12.29 & 7.17 & 4.77 & 3.34 & 2.46 & 2.03 & 1.43 & 0.19 \\ 
Profit Sharing                &  (11.99)   & (19.97) & (7.79) & (3.07) & (1.79) & (1.19) & (0.84) & (0.61) & (0.51) & (0.36) & (0.05) \\ 
\cmidrule{2-12} 
\addlinespace 
\textbf{Net Cash Flows}       & \textbf{35.98} & \textbf{59.91} & \textbf{23.37} & \textbf{9.21} & \textbf{5.37} & \textbf{3.58} & \textbf{2.51} & \textbf{1.84} & \textbf{1.52} & \textbf{1.07} & \textbf{0.14} \\ 
\addlinespace 
PV of Cash Flows              & \textbf{134.41} & 103.35 & 45.61 & 23.35 & 14.84 & 9.94 & 6.68 & 4.38 & 2.67 & 1.20 & 0.14 \\ 
\bottomrule 
\end{tabularx} 
\end{table}

Initial oil price is set at \$70.62 per barrel, based on the parameter values obtained for the Schwartz–Smith two-factor model as presented in Table \ref{tab4}. After 10,000 Monte Carlo iterations, which account for production uncertainty as illustrated in Figure \ref{fig21}, the Variable Operating Cost Rate modeled as a MGB, and the Oil Price following the Schwartz–Smith two-factor model, the project value with real options is obtained. By incorporating the managerial flexibilities of continuation, buyout, and divest options between Year~1 and Year~10, the present value of the project increases to approximately \$148 million, as shown in Figure~\ref{fig22}. This corresponds to an absolute gain of \$14 million, or approximately 10.5\% relative to the base case. Such an increase highlights the economic importance of managerial flexibility under uncertainty.

\begin{figure}[H]
\centering
\begin{minipage}{0.95\textwidth}
  \includegraphics[width=\textwidth]{density_NPV_2_egg.eps}
  \caption{PV of the Egg model (a) without options and (b) with options from Year~1 to Year~10.}
  \label{fig22}
\end{minipage}
\end{figure}

Figure \ref{fig23} presents the probability of exercising each option for the Egg model across the decision horizon from Year~1 to Year~10. At the initial stage (Year~1), the buyout option has probability equal to one, reflecting the starting point of the project investment. From Year 1 onward, the continue option dominates, with high probability values in the early years, indicating that continuation is the preferred managerial strategy under most realizations.

\begin{figure}[H]
\centering
\begin{minipage}{0.95\textwidth}
  \includegraphics[width=\textwidth]{options_prob_egg.eps}
  \caption{Probability of each option for the Egg model in each year.}
  \label{fig23}
\end{minipage}
\end{figure}

As time progresses, the probability of divest gradually increases, especially after Year~5, reaching significant levels between Years~7 and 9. This pattern reflects the decline in production and revenues observed in the base case, which makes divestment economically attractive in later stages. Figure~\ref{fig23} therefore illustrates the dynamic trade-off between continuation and divestment, where early continuation maximizes project value under uncertainty, while divestment becomes more relevant as production declines.

Figure~\ref{fig24} shows the present values of the Egg model when options are exercised at different times. Teal line represents the case in which options can be exercised cumulatively over the decision horizon. In this case, the option value does not increase substantially when compared with the results obtained for the BDH problem (see Figure~\ref{fig19}). This is mainly due to the relatively small production volumes of the Egg model and its rapid production decline, which limit the incremental value of flexibility.

\begin{figure}[H]
\centering
\begin{minipage}{0.75\textwidth}
  \includegraphics[width=\textwidth]{graph_options_egg.eps}
  \caption{Present values of the Egg model for options exercised at different times.}
  \label{fig24}
\end{minipage}
\end{figure}

Darkred line in the Figure~\ref{fig24}  illustrates the case where the option is available in just one year (Year~1, Year~2, …, Year~10). In this setting, the option value is higher in the initial years of production, but it decreases steadily as time progresses, reflecting the diminishing economic potential of late-stage decisions. Nevertheless, the project value with options remains consistently higher than the base case without options, represented by the dashed goldenrod line at \$134 million. Results presented are consistent with the expected behavior of petroleum projects under uncertainty and clearly demonstrate the economic value of managerial flexibility. By explicitly considering alternative decision paths, the analysis highlights the importance of ROA as a complement to traditional DCF methods. These findings reinforce that incorporating real options into project evaluation provides a more realistic and robust approach for assessing the value of petroleum E\&P investments.

\section{Conclusion}\label{sec6}

This work developed and applied an integrated framework for the valuation of petroleum E\&P projects under geological and market uncertainties using ROA. Building on the BDH problem and extending it to the 3D Egg model, the analysis incorporated multiple layers of uncertainty through stochastic modeling of oil prices, operating costs, and production profiles. The use of MCS and the LSM method allowed the determination of optimal decision strategies and the quantification of option values across a wide range of scenarios.

The results show that the incorporation of managerial flexibility consistently enhances project value relative to the traditional DCF approach. In the Egg model case, although production volumes are relatively small and decline rapidly, the availability of continuation, buyout, and divest options increased the project value from \$134 million (base case without options) to \$148 million when real options were considered. This improvement, amounting to an additional \$14 million or approximately 10.5\%, underscores the economic significance of flexibility even for onshore fields with modest production.

Furthermore, the probability maps, value trajectories, and decision strategies obtained reinforce the dynamic nature of project management under uncertainty. They highlight that continuation tends to dominate in early years when production levels are higher, while divestment becomes increasingly attractive in later years as production declines. These results are consistent with the theoretical principles of real options, illustrating how flexibility can be exercised at different stages of a project to both mitigate downside risks and capture upside opportunities.

Overall, the study demonstrates that ROA provides a more realistic and robust approach for project evaluation compared to static valuation methods. By explicitly accounting for uncertainty and flexibility, ROA equips decision-makers with better tools to navigate the inherent risks of petroleum projects, supporting strategies that maximize value creation. These insights are not only relevant for academic research but also for practical decision-making in the oil and gas industry, where uncertainty is pervasive and flexibility often determines the success of investment decisions.
