...
Figure \ref{fig18} displays the decision maps corresponding to four distinct time periods. In these maps, goldenrod color regions represent where the divest option is optimal, while teal color regions indicate that the continue alternative is preferred. As time advances, the area where the divest decision is optimal expands, covering a larger portion of the map. These results are consistent with the findings presented in Figure \ref{fig17}.
...
Figure \ref{fig20} shows the oil production rates for the four main producing wells, based on the results of 101 reservoir simulation runs for the 3D Egg model (see Figure \ref{fig3}). Each subplot displays the production curves for all realizations in teal color and highlights the average production curve in darkred color for each well. This approach visually illustrates the range of production uncertainty and the expected (mean) production behavior.
...
\begin{minipage}{0.95\textwidth}
  ...
  \caption{Production Egg field and PDF/CDF.}
  \label{fig21}
\end{minipage}
\end{figure}
