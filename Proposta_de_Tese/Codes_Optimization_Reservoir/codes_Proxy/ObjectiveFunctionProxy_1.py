...
ModelDir = os.path.join(MAINDIR, "Egg - Two")


def objective(ga_instance, solution, solution_idx):
    solution = np.round(solution).astype(int)
    os.chdir(MAINDIR)
    model = load_model("TrainedMLP - First Period.hdf5", compile=False) # You train an MLP for each period. Save it and load it here for each period
    ...
    with open("Scaler - First Period", "rb") as file:
    ...
    max_cop = 14510.75  # Maximum cumulative oil production. It will differ if simulations change. For first period enter 14510.75. This should be determined based on your simulations
