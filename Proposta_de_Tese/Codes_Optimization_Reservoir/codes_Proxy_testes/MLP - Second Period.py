# -*- coding: utf-8 -*-
"""
Created on Sun Oct 13 00:17:56 2024
...
data = pd.read_csv("Second-Period.csv").dropna()

#perm_files = glob.glob(r"your\working\directory\Realizations\*.INC")
perm_files = glob.glob("/mnt/c/Users/calva/OneDrive/Documentos/Doutorado/Proposta_de_Tese/Codes_Optimization_Reservoir/Realizations/*.INC")

perms = np.zeros((4500,60,60,7))
n=0
for file in perm_files:
    ...
    for k in range(perms.shape[3]):
        for j in range(perms.shape[2]):
            perms[n*1500:(n+1)*1500, :, j, k] = x[j*10:(j+1)*10,:].ravel()
...
save_dir = "/mnt/c/Users/calva/OneDrive/Documentos/Doutorado/Proposta_de_Tese/Codes_Optimization_Reservoir/codes_Proxy"
plt.savefig(f"{save_dir}/permeability_layer0_Second_Period.png", dpi=300)
plt.close()
...
with open("Scaler - Second Period", "wb") as file:
...
if os.path.exists("TrainedMLP - Second Period.hdf5"):
    model = load_model("TrainedMLP - Second Period.hdf5", compile=False)
else:
    ...
    model.save("TrainedMLP - Second Period.hdf5")
...
plt.text(x=0.7, y=0.9, s="Training data", fontdict=dict(fontsize=12))
plt.title("True vs. Predicted Values with $R^2$ Score (Train)")
fig_train.tight_layout()
fig_train.savefig("Crossplot Train - Period 2.jpg", dpi=300, format="jpg")
plt.close(fig_train)

# --------------------------------------------
# ---- CROSSPLOT DO TESTE (Period 2) ----------
# --------------------------------------------
...
plt.text(x=0.7, y=0.9, s="Testing data", fontdict=dict(fontsize=12))
plt.title("True vs. Predicted Values with $R^2$ Score (Test)")
fig_test.tight_layout()
fig_test.savefig("Crossplot Test - Period 2.jpg", dpi=300, format="jpg")
plt.close(fig_test)
...
plt.tight_layout()
plt.savefig("Loss_vs_Epochs_Period_2.jpg", dpi=300)
plt.close(fig_val)
with open("History - Period 2", "wb") as file:
    pickle.dump(history.history, file)

# In[]
np.save("Test Predictions - Period 2", y_pred_test)
np.save("Train Predictions - Period 2", y_pred_train)
np.save("Validation Predictions - Period 2", y_pred_val)
