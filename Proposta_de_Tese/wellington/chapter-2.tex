...
A Figura~\ref{fig:eta2_empirical} apresenta o resultado de uma simulação empírica comparando o valor teórico e o valor estimado de $\eta^2(X|S)$ para diferentes níveis de correlação $\rho$. O código \textit{Python} gera amostras bivariadas normais com correlação controlada e calcula, para cada valor de $\rho$, a fração da variância total de $X$ explicada por $S$, utilizando a definição da Equação~\ref{eq:eta2-def}. 
\begin{itemize}
